...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiHome, FiLink } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['pool-owner'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<HostInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/hosts` : null),
    () => getHosts({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Host',
      path: isTenantUser ? `/hosts/view/${data?.[0]?.id}` : '/hosts',
      entity: 'host',
